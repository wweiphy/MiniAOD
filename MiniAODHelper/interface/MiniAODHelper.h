...
std::vector<PATObj1> MiniAODHelper::RemoveOverlaps( const std::vector<PATObj2>& other, const std::vector<PATObj1>& unclean ){
  ...
  for( typename std::vector<PATObj1>::const_iterator iobj1 = unclean.begin(); iobj1!=unclean.end(); ++iobj1 ){
    ...
    unsigned int nSources1 = iobj1->numberOfSourceCandidatePtrs();
	...
	for( unsigned int i1=0; i1<nSources1; i1++ ){
	  ...
	  reco::CandidatePtr source1 = iobj1->sourceCandidatePtr(i1);
    ...
    PATObj1 myObj1 = (*iobj1);
    if( hasOverlaps ){
      math::XYZTLorentzVector original = myObj1.p4();
	...
	original -= overlaper;
      ...
      myObj1.setP4( original );
    }

    cleaned.push_back(myObj1);
  } // end loop over iobj1

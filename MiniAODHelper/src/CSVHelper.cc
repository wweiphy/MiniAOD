...
void CSVHelper::fillCSVHistos(TFile* fileHF, TFile* fileLF, const std::vector< Systematics::Type >& systs)
{
    ...
    h_csv_wgt_lf =
        std::vector< std::vector< std::vector< TF1* > > >(nSys, std::vector< std::vector< TF1* > >(nLFptBins_, std::vector< TF1* >(nLFetaBins_, NULL)));
    ...
    for (size_t iSys = 0; iSys < nSys; iSys++) {
        ...
        if (systematic == "JESUp" || systematic == "JESDown") systematic.ReplaceAll("JES", "JESTotal");
...
TF1* CSVHelper::readHistogram(TFile* file, const TString& name) const
{
    ...
    if (f == nullptr) { throw cms::Exception("BadCSVWeightInit") << "Could not find CSV SF histogram '" << name << "' in file '" << file->GetName() << "'"; }
    // h->SetDirectory(0);
...
{
    if (!isInit_) { throw cms::Exception("BadCSVWeightAccess") << "CSVHelper not initialized"; }
    ...
    for (size_t iJet = 0; iJet < jetPts.size(); iJet++) {
        ...
        if (iPt < 0 || iEta < 0) {
            ...
            throw cms::Exception("BadCSVWeightAccess")
        ...
        if (abs(flavor) == 5) {
            ...
            if (h_csv_wgt_hf.at(iSys).at(iPt)) {
                const double iCSVWgtHF = (csv >= 0.) ? h_csv_wgt_hf.at(iSys).at(iPt)->Eval(csv) : 1.;
            ...
            if (c_csv_wgt_hf.at(iSys).at(iPt)) {
                const double iCSVWgtC = (csv >= 0.) ? c_csv_wgt_hf.at(iSys).at(iPt)->Eval(csv) : 1.;
            ...
            if (h_csv_wgt_lf.at(iSys).at(iPt).at(iEta)) {
                const double iCSVWgtLF = (csv >= 0.) ? h_csv_wgt_lf.at(iSys).at(iPt).at(iEta)->Eval(csv) : 1.;
...
bool CSVHelper::PassesCSV(const pat::Jet& iJet, std::string taggername, const CSVwp iCSVworkingPoint, std::string dataEra)
{
    ...
    if (csvValue > GetWP(dataEra, iCSVworkingPoint, taggername)) { return true; }
    else {
        return false;
    }
}
